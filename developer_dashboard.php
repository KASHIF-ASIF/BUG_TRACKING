...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search'])) {
    ...
    $query = "SELECT Bug.*, Project.project_name, User.username 
              ...
              WHERE Bug.user_id = $logged_in_user_id 
              AND Bug.status != 'Closed' 
              AND (Bug.title LIKE '%$searchKeyword%' OR Bug.description LIKE '%$searchKeyword%')";
} else {
    $query = "SELECT Bug.*, Project.project_name, User.username 
              FROM Bug
              INNER JOIN Project ON Bug.project_id = Project.project_id
              INNER JOIN User ON Bug.user_id = User.user_id
              WHERE Bug.user_id = $logged_in_user_id AND Bug.status != 'Closed'";
}
